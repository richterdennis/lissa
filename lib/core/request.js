...
export default class LissaRequest extends OpenPromise {
	...
	async #execute() {
		...
		let baseURL, url, rejectUnsafeUrl, params, urlBuilder, adapter, reviver;
		({ baseURL, url, rejectUnsafeUrl, params, urlBuilder, adapter, reviver, ...options } = options);

		let offense;
		if (rejectUnsafeUrl && (offense = checkUnsafeUrl(url))) {
			if (typeof rejectUnsafeUrl === 'function') return rejectUnsafeUrl($options);
			throw new TypeError(`Reject a request with unsafe url containing a ${offense}`);
		}
...
function checkUnsafeUrl(url) {
	if (Array.from(url).some((_, i) => url.charCodeAt(i) <= 0x20)) return 'tab, newline, carriage return or control characters';
	if (/[?#]/.test(url)) return 'query or fragment in the path';
	if (url.includes('\\')) return 'backslash';
	if (url.split('/').some(seg => /\.\.?/.test(seg.replaceAll(/%2e/gi, '.')))) return 'dot segment';
}
